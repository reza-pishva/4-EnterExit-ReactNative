...
import EqRequestCover from './PeRequestCover';
import ListingsApi1 from '../../api/Listings15Pe';
import ListingsApi2 from '../../api/Listings16Pe';
import ListingsApi3 from '../../api/Listings17Pe';
import ListingsApi4 from '../../api/Listings18Pe';
import { useIsFocused } from "@react-navigation/native";
import PeTitle from '../../components/PeTitle';
import Screen from '../../components/Screen';

function PeKartabl05({route}) {
    ...
    const loadListings1=async()=>{
        const response= await ListingsApi1.getListing28();
        setListings1(response.data);
    }
    const loadListings2=async()=>{
        const response= await ListingsApi2.getListing29();
        setListings2(response.data);
    }
    const loadListings3=async()=>{
        const response= await ListingsApi3.getListing30();
        setListings3(response.data);
    }
    const loadListings4=async()=>{
        const response= await ListingsApi4.getListing31();
    ...
    useEffect(()=>{
        ...
        const onPress1=()=>{
            navigation.navigate("newRequestForHerasat5Pe",{request1:listings1})
        };
        const onPress2=()=>{
            navigation.navigate("confirmedByHerasat5Pe",{request1:listings2})
        };
        const onPress3=()=>{
            navigation.navigate("notConfirmedByHerasat5Pe",{request1:listings3})
        };
        const onPress4=()=>{
            navigation.navigate("notConfirmedByModir5Pe",{request1:listings4})
        };

    return (
        <Screen>
            {loading ? <PeTitle/> :
          <EqRequestCover top_color="#1A5276"  text="کارتابل مدیر حراست" text_color="white" back_color="#FEF9E7">
            <View>
