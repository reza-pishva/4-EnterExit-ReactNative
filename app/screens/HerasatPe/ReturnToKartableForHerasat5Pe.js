...
function ReturnToKartableForHerasat5Pe({route}) {
  var day=0;
  var month=0;
  var year=0;
  ...
  Alert.alert(
    "",
    "از بازگشت این درخواست اطمینان دارید؟",
    [
      {
        text: "خیر",
 ...
 const handleconfirm = () => {
   const url='http://rezapishva.ir/mob-kartabl4-pe/'+route.params.id_ef;
  fetch(url,{
    method:'get',
    headers:{
      ...
      response.json().then(function(resp){
        Toast.show('این درخواست بازگشت داده شد .', Toast.LONG);
        // console.log(resp)
      })
    })
}; 
      ...
      <Screen>
        ...
        <TouchableOpacity onPress={createTwoButtonAlert} style={styles.itemcontainer3}>
          <View style={{alignItems:'center',marginTop:-5}}>
            <Text style={{color:'white'}}>بازگشت درخواست</Text>
          </View>
        </TouchableOpacity>
        </ScrollView>
...
export default ReturnToKartableForHerasat5Pe;